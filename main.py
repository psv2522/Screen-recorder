...
#record command
def record():
    img = ImageGrab.grab(bbox=(0, 0, 1920, 1080))#takes a screenshot
    frame = np.array(img)#adds images in a array
...
def status_playing(yeter):
    ...
    if status == "stopped":
        ...
        interface.canvas.itemconfig(interface.info, text="Video Saved At Outputs Folder")
        interface.pause["state"] = "disabled"
